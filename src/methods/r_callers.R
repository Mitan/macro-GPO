# todo nb
# this is noisless variant

`qEI_caller` <- function(x, m) {
  ei = qEI(x, m, type="SK",  minimization = FALSE)
  return(ei)
}
...
`model_covariance` <- function(x, m) {
  design <- data.frame(x)
  names(design) <- colnames(m@X)
  output <- predict(object=m, newdata=design, type="SK", cov.compute = TRUE)
  return(output$cov)
}
