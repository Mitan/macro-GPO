...
class RoadDatasetPlotParamStorer:
    ...
    def __init__(self, plotting_type):
        ...
        if plotting_type == PlottingEnum.AverageRewardFull:
            self.y_label_caption = "Avg. normalized output measurements observed by AV"
            self.y_ticks_range = np.arange(0.0, 0.4, 0.1)
            self.y_lim_range = [0.03, 0.34]
            self.legend_loc = 4
            self.legend_size = 15

        elif plotting_type == PlottingEnum.SimpleRegretFull:
            self.y_label_caption = "Simple regret"
            self.y_ticks_range = np.arange(1.5, 3.3, 0.2)
            self.y_lim_range = [1.45, 3.3]
            self.legend_loc = 1
            self.legend_size = 16
