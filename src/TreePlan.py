...
class TreePlan:
    ...
    def __get_gp_prediction(self, current_point, history_locations, history_measurements, cholesky):
        Sigma = self.gp.GPVariance(locations=history_locations, current_location=current_point,
                                   cholesky=cholesky)

        weights = self.gp.GPWeights(locations=history_locations, current_location=current_point,
                                    cholesky=cholesky)
        mu = self.gp.GPMean(measurements=history_measurements, weights=weights)

        return mu[0], Sigma[0, 0]

    # acquizition function
    def __transformed_ei(self, current_point, history_locations, history_measurements, max_measurement, cholesky, t):
    ...
    def __local_penalizer(self, new_x, old_x, mu, var, M):
        # todo note hardcoded
        L = 10
    ...
    def LP(self, x_0, t):
        ...
        for next_state in available_states:
            first_point = next_state.physical_state[0:1, :]
            predicted_val = self.__transformed_ei(current_point=first_point,
        ...
        for num_steps in range(1, self.batch_size):
            ...
            for next_state in available_states:
                ...
                for j in range(num_steps):
                    ...
                    mu_j, Sigma_j = self.__get_gp_prediction(current_point=x_j,
                                                             history_locations=history_locations,
                                                             history_measurements=history_measurements,
