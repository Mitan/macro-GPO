...
class Vis2d:
	...
	def MapPlot(self, grid_extent, ground_truth=None, posterior_mean_before=None, posterior_mean_after=None, posterior_variance_before=None, posterior_variance_after=None, path_points=None, display = True, save_path=None):
		""" 
		...
		if not ground_truth==None:
			im = axes.flat[0].imshow(ground_truth, interpolation='nearest', aspect = 'auto', extent = grid_extent2, cmap = 'Greys', vmin=mmin, vmax=mmax)
			if not path_points==None and path_points:
				for i in xrange(1, len(path_points)):
					axes.flat[0].arrow(path_points[i-1][0], path_points[i-1][1], path_points[i][0]-path_points[i-1][0], path_points[i][1]-path_points[i-1][1], edgecolor='red')

		if not posterior_mean_before==None:
			im = axes.flat[2].imshow(posterior_mean_before, interpolation='nearest', aspect = 'auto', extent = grid_extent2, cmap = 'Greys', vmin=mmin, vmax=mmax)

		if not posterior_mean_after==None:
			im = axes.flat[3].imshow(posterior_mean_after, interpolation='nearest', aspect = 'auto', extent = grid_extent2, cmap = 'Greys', vmin=mmin, vmax=mmax)

		im2 = None
		if not posterior_variance_before==None:
			im2 = axes.flat[4].imshow(posterior_variance_before, interpolation='nearest', aspect = 'auto', extent = grid_extent2)

		if not posterior_variance_after==None:
			im2 = axes.flat[5].imshow(posterior_variance_before, interpolation='nearest', aspect = 'auto', extent = grid_extent2)
...
if __name__ == "__main__":
	...
	v2d = Vis2d()
	v2d.MapPlot(grid_extent=[10, 30, -10, 10], ground_truth=vals, path_points=[(15, 5), (14, 5), (13, 5), (13, 4), (13, 3)])
