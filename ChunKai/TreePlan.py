...
class TreePlan:
	...
	def __init__(self, grid_domain, grid_gap, gaussian_process, action_set=None, max_nodes=None, reward_type="Linear", sd_bonus=0.0, bad_places = None):
		"""
		...
		if action_set==None: self.action_set = ((0, grid_gap), (0, -grid_gap), (grid_gap, 0), (-grid_gap, 0)) # TODO: ensure that we can handle > 2 dimensions
		elif action_set=='GridWithStay': self.action_set = ((0, grid_gap), (0, -grid_gap), (grid_gap, 0), (-grid_gap, 0), (0.0, 0.0))
	...
	def EI(self, x_0):
		...
		*Myopic* implementation of EI (Expected improvement)
		...
		for a in valid_actions:
			...
			sigma = math.sqrt(var)
			mean = self.gp.GPMean(x_next.history.locations, x_next.history.measurements, x_next.physical_state, weights = weights)

			Z = (mean - best_observation)/sigma
			expectedImprov = (mean - best_observation) * norm.cdf(x = Z, loc = 0, scale = 1.0) + sigma * norm.pdf(x = Z, loc = 0, scale = 1.0)
	...
	def ComputeQRandom(self, T, l, x, p, new_st):
		...
		n = math.ceil(2*math.log(0.5-0.5*(p ** self.PEWPEW)) * ((self.l1+new_st.lipchitz)**2) * new_st.variance / (-(l**2)))
		n = max(n, 1)
		if n>1: print n

		sams = np.random.normal(mu, sd, n)

		rrr = [self.ComputeVRandom(T-1, l, self.TransitionH(x, sam), p ** ((1-self.PEWPEW)/n), new_st) + self.reward_sampled(sam) for sam in sams]
		avg = np.mean(rrr)
	...
	def PreprocessLipchitz(self, node, isRoot=False):
		...
		nl = node.ss.locations.shape[0] # number of elements PRIOR to adding current location. Ie. the size of the weight space
	...
	def PreprocessPartitions(self, node, err_allowed, isRoot=False):
		...
		if node.n == 0: translated=1 # special case where we may double count both tail evaluations
		else: translated = node.n+2 # taking into account both tail evaluations (2 of them)
		...
		for a, c in node.children.iteritems(): t += self.PreprocessPartitions(c, err_allowed)
		
		else: return t * translated + translated
	...
	def EstimateV_tilde(self, T, l, gamma, x, new_st):
		...
		if num_partitions == 2: vLeftTailVal = vRightTailVal # Quick hack for cases where the algo only requires us to us MLE (don't need to repeat measurement at mean of gaussian pdf)
		else: 
