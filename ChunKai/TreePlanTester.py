...
class TreePlanTester:
	...
	def Test(self, num_timesteps_test, debug = True, visualize = False, action_set=None, save_per_step = True, save_folder = "default_results/", MCTS=True, MCTSMaxNodes=10**15, cheat=False, cheatnum=0, Randomized=False, special = None):
		""" Pipeline for testing 
		@param num_timesteps_test - int, number of timesteps we should RUN the algo for. Do not confuse with search horizon
		"""

		x_0 = AugmentedState(self.initial_physical_state, initial_history=History(self.past_locations, self.past_measurements))
		state_history = [x_0]
		...
		for time in xrange(num_timesteps_test):
			tp = TreePlan(self.grid_domain, self.grid_gap, self.gp, action_set=action_set, reward_type=self.reward_model, sd_bonus=self.sd_bonus, bad_places = self.bad_places)
			...
			if time == 0 and cheat:
				...
				if not MCTS:
					...
					bounds, a, nodes_expanded = tp.MCTSExpand(self.epsilon, self.gamma, x_0, self.H, max_nodes=MCTSMaxNodes)
			else:
				# Use random sampling
	...
	def Visualize(self, state_history, display=True, save_path=None):
		...
		posterior_mean_before = np.vectorize(lambda x, y: self.gp.GPMean(state_history[-2].history.locations, state_history[-2].history.measurements, [x, y]))
		posterior_mean_after = np.vectorize(lambda x, y: self.gp.GPMean(state_history[-1].history.locations, state_history[-1].history.measurements, [x, y]))
		posterior_variance_before = np.vectorize(lambda x, y: self.gp.GPVariance2(state_history[-2].history.locations, [x, y]))
		posterior_variance_after = np.vectorize(lambda x, y: self.gp.GPVariance2(state_history[-1].history.locations, [x, y]))

		# Plot graph of locations
		vis = Vis2d()
		vis.MapPlot(grid_extent=[self.grid_domain[0][0], self.grid_domain[0][1], self.grid_domain[1][0], self.grid_domain[1][1]],
...
def SanityCheck():
	...
	TPT.InitEnvironment(environment_noise=0.1, model=lambda xy: multivariate_normal(mean=[0,0], cov=[[1,0],[0,1]]).pdf(xy))
	TPT.InitPlanner(grid_domain=((-10, 10), (-10, 10)), grid_gap=0.2, gamma=1, epsilon=100.0, H=5)
	TPT.InitTestParameters(initial_physical_state=np.array([1.0, 1.0]), past_locations=np.array([[-1.0, -1.0], [1.0, 1.0]]))
...
def Exploratory(grid_gap_, epsilon_=100.0):
	...
	TPT=TreePlanTester()
	TPT.InitGP(length_scale=[1.5, 1.5], signal_variance=1, noise_variance=0.1)
	TPT.InitEnvironment(environment_noise=0.1, model=lambda xy: multivariate_normal(mean=[0,0], cov=[[64,0],[0,16]]).pdf(xy))
...
def Random(grid_gap_=0.05, length_scale=(0.1, 0.1), epsilon_ = 5.0, depth=3, num_timesteps_test = 20, 
			...
			preset=False, action_set=None, MCTS = True, MCTSMaxNodes = 10 ** 15, reward_model="Linear", cheat=False, cheatnum=0, Randomized=False, sd_bonus=0.0,
			special = None):
	...
	TPT.InitTestParameters(initial_physical_state=np.array([0.5, 0.5]), past_locations=np.array([[0.5, 0.5]]) if not preset else np.array([[0.25, 0.25], [0.25, 0.75], [0.75, 0.75], [0.75, 0.25], [0.5, 0.5]]))
	return TPT.Test(num_timesteps_test=num_timesteps_test, debug=True, visualize=False, save_folder=save_folder, action_set=action_set, save_per_step=save_per_step, MCTS = MCTS, MCTSMaxNodes=MCTSMaxNodes, cheat=cheat, cheatnum=cheatnum, Randomized=Randomized, special = special)
...
def TestRealData(locations, values, length_scale, signal_variance, noise_variance, mean_function, grid_domain, start_location, 
	...
	TPT=TreePlanTester(simulate_noise_in_trials=False, reward_model = reward_model, sd_bonus = sd_bonus, bad_places = bad_places)
	TPT.InitGP(length_scale=length_scale, signal_variance=signal_variance, noise_variance=noise_variance, mean_function = mean_function)
	TPT.InitEnvironment(environment_noise=noise_variance, model=m)
	...
	# For normal
	return TPT.Test(num_timesteps_test=num_timesteps_test, debug=True, visualize=False, action_set=None, save_folder=save_folder, save_per_step=save_per_step, MCTS = MCTS, MCTSMaxNodes=MCTSMaxNodes, Randomized=Randomized, special = special)
300
if __name__ == "__main__": 
	...
	for i in xrange(15,26):
		Random(length_scale=(0.1, 0.1), epsilon_=10**10, seed=i, depth=4, save_folder=save_trunk+"/seed"+str(i)+"/", preset=False)
		#Transect(seed=i)
