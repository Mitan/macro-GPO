...
class mutil:
    ...
    def FindSmallestPartition(self, max_error):
        ...
        assert False, "Unable to find number of partitions, only preprocessed to length: %d with tolerance of %f. Required tolerance: %f" % (
            (len(self.sol) - 1), self.min_err[-1], max_error)
    ...
    def ComputeMaximumError(self, n):
        ...
        # One modified
        func2 = lambda x: ((norm.cdf(x) - 0.5) * 2.0 * x / n + math.sqrt(2.0 / math.pi) * math.exp(
            -0.5 * x * x)) - 2 * x * norm.cdf(-x)
    ...
    def DistributionForTransition(self, valid_actions, best_action):
        # hardcoded probability for main action
        p_best = 0.8
...
if __name__ == "__main__":
    ...
    print c.DistributionForTransition([1, 2, 3, 4, 5], 1)
    print c.min_err
    print c.sol
