...
def drawPlot(all_locations, values, path_points, save_path, current_step, batch_size):
    ...
    circle = plt.Circle(start_point, 0.8, lw=4.0, color='black', fill=False)
    axes.add_artist(circle)
    circle = plt.Circle(fake_point, 0.8, lw=2.0, color='black', fill=False)
    axes.add_artist(circle)
    colors = ['red', 'blue', 'green', 'black', 'brown']
    for i in range(current_step+1):
        # color the batch
        for j in range(batch_size):
            circle = plt.Circle(path_points[2 + i * batch_size + j], 0.8, lw=1.0, color=colors[i], fill=False)
            axes.add_artist(circle)
